...
class TaskStream:
    ...
    async def send_to_dlq(self, task: Task, reason: str) -> None:
        ...
        await self.ack(task.stream, STAGE_GROUPS[task.stream], task.message_id)
        log.error(
            "task dead-lettered",
            stream=task.stream,
    ...
    # Reclaiming stalled messages (XPENDING + XCLAIM, dead-lettering after
    # MAX_ATTEMPTS) lands on Day 3. It slots in here, beside consume(), and
    # reads the per-stage thresholds from settings.
